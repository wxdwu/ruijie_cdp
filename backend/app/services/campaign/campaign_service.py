"""营销活动看板服务（campaign board）。

原 campaign 路由内约 1200 行均为 DWS 查询/聚合/响应整形逻辑，现已下沉到此 service：
路由层只负责参数校验与委托调用。本模块对外暴露与端点一一对应的函数
（首个参数为 db Session），便于复用与单测。
"""
...
def _campaign_filters(
    ...
    *,
    ...
    if _is_key_account_campaign(campaign_tag):
        ...
        where_parts.append(f"c.campaign_tag = :campaign_tag")
        params["campaign_tag"] = campaign_tag
    if industry:
        where_parts.append(f"c.industry = :industry")
        params["industry"] = industry
    if stage:
        where_parts.append(f"c.purchase_stage = :stage")
        params["stage"] = stage
    if owner:
        where_parts.append(f"c.owner_name = :owner")
        params["owner"] = owner
    if keyword:
...
def _interaction_filters(
    *,
    ...
    where_parts = [f"i.customer_name IS NOT NULL", f"i.customer_name != ''"]
    params: Dict[str, Any] = {}
    ...
    if _is_key_account_campaign(campaign_tag):
        where_parts.append(
            ...
            f"AND ka.`重客名称` = i.customer_name)"
        )
        if industry:
            where_parts.append(
                "EXISTS (SELECT 1 FROM dws_customer_360 c "
                f"WHERE c.customer_name = i.customer_name "
                "AND c.campaign_tag = :key_account_source_project "
        ...
        customer_parts = [f"c.customer_name = i.customer_name"]
        if campaign_tag:
...
def get_campaign_kpis(
    ...
    total_customer_where, total_customer_params = _campaign_filters(
        db, campaign_tag=campaign_tag, industry=industry,
    )
...
def get_funnel_distribution(
    ...
    customer_scope = _customer_scope_sql(campaign_tag)
    where_parts, params = _campaign_filters(db, campaign_tag=campaign_tag, industry=industry)
    rows = db.execute(
        text(
...
def get_tag_signals(
    ...
    customer_scope = _customer_scope_sql(campaign_tag)
    customer_where, customer_params = _campaign_filters(db, campaign_tag=campaign_tag, industry=industry)
...
def get_campaign_overview(
    ...
    base_where, base_params = _campaign_filters(db, campaign_tag=campaign_tag, industry=industry)
    active_where, active_params = _campaign_filters(
        db, campaign_tag=campaign_tag, industry=industry,
        channel=channel, start_date=start_date, end_date=end_date,
        require_interaction_window=True,
