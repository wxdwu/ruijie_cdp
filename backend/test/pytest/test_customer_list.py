...
def test_standard_region_filter_matches_plain_and_area_suffix(mock_db):
    ...
    sql, params = mock_db.calls[0]
    assert "region IN (:region_exact, :region_area)" in sql
    assert params["region_exact"] == "广东"
    assert params["region_area"] == "广东区域"
...
def test_canonical_channel_filter_accepts_code_and_chinese_label(mock_db):
    ...
    sql, params = mock_db.calls[0]
    assert "EXISTS (SELECT 1 FROM dws_interaction_detail interaction_channel" in sql
    assert "interaction_channel.customer_name = dws_customer_360.customer_name" in sql
    assert "interaction_channel.channel IN" in sql
...
def test_other_channel_filter_excludes_known_and_empty_values(mock_db):
    ...
    sql, params = mock_db.calls[0]
    assert "interaction_channel.channel IS NOT NULL" in sql
    assert "TRIM(interaction_channel.channel) != ''" in sql
    assert "interaction_channel.channel NOT IN" in sql
...
def test_key_account_filter_supports_keyword_and_pagination(mock_db):
    ...
    assert "ka.`重客名称` LIKE :keyword" in count_sql
    assert count_params == {
...
def test_key_account_filter_applies_enriched_customer_filters(mock_db):
    ...
    _, data_params = mock_db.calls[1]
    assert "LEFT JOIN dws_customer_360" in count_sql
    assert "c.industry = :industry" in count_sql
    assert "c.region IN (:key_account_region_exact, :key_account_region_area)" in count_sql
    assert "c.owner_name = :owner" in count_sql
    assert "dws_interaction_detail" in count_sql
    assert "EXISTS (SELECT 1 FROM dws_interaction_detail interaction_channel" in count_sql
    assert "interaction_channel.customer_name = ka.`重客名称`" in count_sql
    assert "interaction_channel.channel IN" in count_sql
    assert count_params["industry"] == "软件"
    assert count_params["key_account_region_exact"] == "广东"
    assert count_params["key_account_region_area"] == "广东区域"
    assert count_params["owner"] == "张三"
    assert count_params["interaction_min"] == 3
    assert {count_params["key_account_channel_0"], count_params["key_account_channel_1"]} == {"email", "邮件"}
    assert data_params["limit"] == 20
...
def test_key_account_keyword_and_channel_stay_within_latest_snapshot(mock_db):
    ...
    assert "ka.`重客名称` LIKE :keyword" in count_sql
    assert "interaction_channel.customer_name = ka.`重客名称`" in count_sql
    assert params["keyword"] == "%山东魏桥%"
    assert {params["key_account_channel_0"], params["key_account_channel_1"]} == {"web", "官网"}
...
def test_standard_project_filter_options_are_scoped(mock_db):
    ...
    sql, params = mock_db.calls[0]
    assert "c.campaign_tag = :filter_special_project" in sql
    assert params["filter_special_project"] == "企业彩光ICT"
...
def test_key_account_export_uses_enriched_query_and_filters(client, mock_db):
    ...
    assert "c.industry = :industry" in sql
    assert "ka.`重客名称` ASC" in sql
    assert params["industry"] == "制造业"
