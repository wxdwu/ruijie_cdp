...
es_crud 路由通过 `app.services.elasticSearch.es_crud.*` 调用，es_sync 路由通过
`app.services.elasticSearch.es_sync.*` 调用；直接 patch 这些模块级函数即可。
"""
...
def test_es_get_doc(client: TestClient, monkeypatch):
    monkeypatch.setattr("app.services.elasticSearch.es_crud.es_get", lambda index, doc_id: {"name": "x"})
...
def test_es_get_doc_not_found_404(client: TestClient, monkeypatch):
    monkeypatch.setattr("app.services.elasticSearch.es_crud.es_get", lambda index, doc_id: None)
    r = client.get("/api/admin/es/cdp_customer_360/abc")
    assert r.status_code == 404


def test_es_batch_create(client: TestClient, monkeypatch):
    monkeypatch.setattr("app.services.elasticSearch.es_crud.es_bulk_create", lambda index, docs, batch_size: 3)
    r = client.post(
...
def test_es_search(client: TestClient, monkeypatch):
    monkeypatch.setattr("app.services.elasticSearch.es_crud.es_search", lambda index, query: {"hits": {}})
...
def test_es_update(client: TestClient, monkeypatch):
    monkeypatch.setattr("app.services.elasticSearch.es_crud.es_update", lambda index, doc_id, doc: None)
...
def test_es_batch_update(client: TestClient, monkeypatch):
    monkeypatch.setattr("app.services.elasticSearch.es_crud.es_bulk_update", lambda index, docs, batch_size: 2)
    r = client.post(
...
def test_es_delete(client: TestClient, monkeypatch):
    monkeypatch.setattr("app.services.elasticSearch.es_crud.es_delete", lambda index, doc_id: True)
...
def test_es_batch_delete(client: TestClient, monkeypatch):
    monkeypatch.setattr("app.services.elasticSearch.es_crud.es_bulk_delete", lambda index, ids, batch_size: 2)
...
def test_es_delete_by_query(client: TestClient, monkeypatch):
    monkeypatch.setattr("app.services.elasticSearch.es_crud.es_delete_by_query", lambda index, query: 5)
    r = client.post(
...
def test_es_full_sync(client: TestClient, monkeypatch):
    monkeypatch.setattr(
        "app.services.elasticSearch.es_sync.run_es_full_sync", lambda: {"synced": 100}
    )
...
def test_es_increment_sync(client: TestClient, monkeypatch):
    monkeypatch.setattr(
        "app.services.elasticSearch.es_sync.run_es_incremental_sync", lambda: {"synced": 10}
    )
...
def test_es_health(client: TestClient, monkeypatch):
    monkeypatch.setattr(
        "app.services.elasticSearch.es_sync.get_es_health",
