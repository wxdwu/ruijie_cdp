...
import { SQLBOT_SDK_URL, SQLBOT_EMBEDDED_ID } from '../config.js'

let sqlbot_embedded_timer: number | null = null

onMounted(() => {
  console.log('[SQLBot] 开始加载 SQLBot SDK...')
  console.log('[SQLBot] SDK URL:', SQLBOT_SDK_URL)
  console.log('[SQLBot] Embedded ID:', SQLBOT_EMBEDDED_ID)

  // 动态加载 SQLBot SDK
  const script = document.createElement('script')
  script.defer = true
  script.async = true
  script.src = SQLBOT_SDK_URL
  ...
  script.onerror = () => {
    ...
    console.error('  1. SQLBot 服务是否正常运行')
