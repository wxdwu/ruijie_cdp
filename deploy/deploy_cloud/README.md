# Ruijie CDP — 公网部署（本机 MySQL + 后端 + 前端）

> 本目录 `deploy/deploy_cloud/` 为公网/独立服务器部署方案：**本机 docker MySQL + 后端 + 前端(Nginx)**，
> 由 `docker-compose.yml` 一键拉起，适配「不同公网 IP 本地部署数据库和前后端」。
> 局域网部署（仅前后端、连云 MySQL）请见同级 `../deploy_local/` 目录。
>
> 本目录内容即原 `deploy/` 下的生产部署套件，仅因目录层级加深调整了构建上下文与文件路径。
...
| 部署 | `deploy/deploy_cloud/deploy.sh` | 本地进程，不用 docker | `sudo bash deploy/deploy_cloud/deploy.sh up`，docker 全套 |

后端配置优先级：**进程环境变量 > `.env.{APP_ENV}` 文件 > `config.py` 默认值**。
默认 `APP_ENV=development`（本地开发最频繁）；生产环境由 `deploy/deploy_cloud/docker-compose.yml` 显式设置 `APP_ENV=production`，故 docker 部署仍走生产配置。
